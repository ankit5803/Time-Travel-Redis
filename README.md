<div align="center">
  ...
  **A custom, in-memory caching database built from scratch using raw TCP sockets, Python \syncio\, and the REdis Serialization Protocol (RESP).**
...
* **?? Layer 1: The TCP Engine (Backend)** 
  A custom event-loop server handling non-blocking I/O multiplexing via \syncio\. It bypasses HTTP entirely, reading and writing raw RESP byte-streams directly over port \6379\.
...
* **(1)$ Hash Map Store:** Instant read/write operations utilizing Python's internal dictionary structures.
